...
function readLanes(): LightningLane[] {
  return parseLightningLanes(window.localStorage.getItem(STORAGE_KEY));
}
...
function renderLightningLaneConflictNote(lanes: LightningLane[], planPanel: Element) {
  ...
  const text = lightningLaneConflictNote(lanes);
  if (!text) return;

  const note = document.createElement("div");
  note.className = "plan-note lightning-lane-conflict-note";
  const label = document.createElement("strong");
  label.textContent = "LL check:";
...
function updateAddReadyState(form: HTMLFormElement) {
  ...
  const ready = isValidLightningLane({ id: "pending", name, start, end, used: false });
...
function renderLightningLaneTracker() {
  ...
  titleLabel.textContent = "Lightning Lane tracker";
  const titleHint = document.createElement("span");
  titleHint.textContent = nextLightningLaneHint(lanes);
  ...
  if (!lanes.length) {
    ...
    sortLightningLanesByUrgency(lanes).forEach((lane) => {
